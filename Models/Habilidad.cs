        ...
        public AnexGRIDResponde Listar(AnexGRID grid, int Usuario_id)
        {
            try
            {
                using (var ctx = new AppContext())
                {
                    ...
                    //Elegimos tabla a paginar (Habilidad)
                    var query = ctx.Habilidad.Where(x => x.Usuario_id == Usuario_id); //filtro la habilidad x usuario

                    //CAMPOS DE LA TABLA: Nombre, Dominio
                    ...
                    var Habilidads = query.Skip(grid.pagina) //Trae registros a partir de... 
